"""
Enhanced Interactive Python Tutor application.
Orchestrates all modules with strict lesson progression and interactive features.
"""
...
class PythonTutor:
    """Enhanced Interactive Python Tutor with strict progression control"""
    ...
    def start_current_lesson(self):
        """Start or continue the current lesson with enhanced interactive features"""
        ...
        if self.progress_tracker.is_lesson_completed(self.current_lesson):
            ...
            # Check if associated project is completed before allowing next lesson
            if not self.progress_tracker.is_project_completed(self.current_lesson):
                print(f"📝 However, Project {self.current_lesson} is not yet completed.")
                print("💡 Complete the project before moving to the next lesson!")
                ...
                if choice == "project":
                    ...
                    self._review_completed_lesson(self.current_lesson)
                
                return
            else:
                # Both lesson and project completed, can advance
                choice = Utils.get_user_choice(
        ...
        # Show lesson content with interactive practice
        print(f"\n📖 STARTING LESSON {self.current_lesson}")
        
        # Use enhanced interactive lesson display
        if not self.lesson_manager.show_lesson_content_interactive(self.current_lesson, self.progress_tracker):
            input("Press Enter to continue...")
            return

        # Automatically run interactive exercises (built into lesson content now)
        print(f"\n🎯 Running interactive exercises for Lesson {self.current_lesson}...")
        ...
        if self.lesson_manager.run_interactive_exercises(self.current_lesson, self.progress_tracker):
            ...
            # Automatically prompt for project
            print("\n" + "="*50)
            print("🚀 NEXT STEP: PROJECT TIME!")
            print("="*50)
            print("Now it's time to apply what you've learned in a hands-on project.")
            print("💡 You must complete the project to unlock the next lesson!")
            
            if input("Ready to work on the project? (y/n): ").lower() == 'y':
                self.work_on_specific_project(self.current_lesson)
            else:
                print("📌 Remember: Complete the project to continue to the next lesson!")

        input("Press Enter to continue...")

    def _review_completed_lesson(self, lesson_num):
        """Review a completed lesson"""
        print(f"\n🔄 REVIEWING LESSON {lesson_num}")
        print("="*40)
        
        # Show lesson content without interactive elements for review
        ...
        if lesson_file.exists():
            ...
            print(f"❌ Lesson {lesson_num} content not found!")
        
        input("\nPress Enter to continue...")

    def work_on_project(self):
        """Work on projects with strict lesson completion requirements"""
        Utils.clear_screen()
    ...
    def work_on_specific_project(self, project_num):
        """Work on a specific project with lesson completion check"""
        
        # Strict check: lesson must be completed first
        if not self.progress_tracker.is_lesson_completed(project_num):
            print(f"🔒 Project {project_num} is locked!")
            print(f"💡 You must complete Lesson {project_num} first.")
            print(f"📚 Go back to the main menu and start Lesson {project_num}.")
            input("Press Enter to continue...")
            return

        # Show project information with vim guidance
    ...
    def project_mode(self, project_num, project_dir):
        ...
        try:
            ...
            while True:
                print("\nAvailable commands:")
                print("  edit - Open your project in vim (with template)")
                print("  run - Run your project")
                print("  test - Test your project")
                print("  submit - Submit and advance to next lesson")
                ...
                if command == "quit":
                    ...
                    # Use enhanced project manager method
                    self.project_manager.open_project_in_vim_with_guidance(project_dir)
                elif command == "run":
    ...
    def _show_project_help(self):
        """Show detailed project mode help"""
        print("\n🆘 PROJECT MODE HELP")
        print("=" * 30)
        print("You're working on a coding project. Here's how to succeed:")
        print("1. Use 'edit' to write your code in main.py")
        print("   • Template code is loaded automatically")
        print("   • Follow the TODO comments in the template")
        print("   • Vim hints are shown each time you edit")
        ...
        print("   • This will unlock the next lesson!")
    ...
    def submit_project(self, project_num, project_dir):
        """Submit project with strict quality checks and lesson advancement"""
        ...
        if passed:
            ...
            # IMPORTANT: Advance to next lesson only after project completion
            if project_num == self.current_lesson:
                if self.current_lesson < 30:
                    self.current_lesson += 1
                    self.progress_tracker.set_current_lesson(self.current_lesson)
                    print(f"🔓 Lesson {self.current_lesson} is now unlocked!")
                    print("🎯 Return to main menu to continue your learning journey!")
                else:
                    print("🎓 Congratulations! You've completed the entire Python Tutor!")
    ...
    def choose_project_menu(self):
        """Menu to choose a specific project with lesson completion checks"""
        print("\n🎯 CHOOSE PROJECT")
        print("=" * 30)
    ...
    def view_progress(self):
        ...
        if self.current_lesson <= 30:
            if not self.progress_tracker.is_lesson_completed(self.current_lesson):
                print(f"\n📚 Next: Complete Lesson {self.current_lesson}")
            elif not self.progress_tracker.is_project_completed(self.current_lesson):
                print(f"\n🚀 Next: Complete Project {self.current_lesson}")
                print("💡 Project completion unlocks the next lesson!")
            else:
                print(f"\n🎯 Ready for Lesson {self.current_lesson + 1}!")
    ...
    def settings_menu(self):
        ...
        while True:
            ...
            if choice == "1":
                ...
                self.lesson_manager.show_vim_hints()
                input("Press Enter to continue...")
            elif choice == "6":
                break
            else:
